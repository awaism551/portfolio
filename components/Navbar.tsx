...
const { nav: links, personal } = portfolio;
...
export default function Navbar() {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "border-b backdrop-blur-md" : ""}`}
      style={{ background: scrolled ? "rgba(10,10,15,0.85)" : "transparent", borderColor: scrolled ? "var(--border)" : "transparent" }}
    >
      ...
      {/* Mobile menu */}
      {open && (
        <div className="md:hidden border-t px-6 py-4 flex flex-col gap-1" style={{ background: "var(--surface)", borderColor: "var(--border)" }}>
          {links.map(({ href, label }) => (
            <a
              key={href}
              href={href}
